...
//	NSString *aTitle = [anObject objectForKey:@"Title"];
...
- (BOOL)equalToBrand:(Brand *)aBrand {
	if (aBrand == nil) {
		return NO;
	}
    
	if (aBrand.object && self.object) {
		// We have a PFObject inside the Store, use that instead.
		if ([aBrand.object.objectId compare:self.object.objectId] != NSOrderedSame) {
			return NO;
		}
		return YES;
	} else {
		...
		if ([aBrand.title    compare:self.title]    != NSOrderedSame
