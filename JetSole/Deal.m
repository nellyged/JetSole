...
- (BOOL)equalToDeal:(Deal *)aDeal {
	if (aDeal == nil) {
		return NO;
	}
    
	if (aDeal.object && self.object) {
		// We have a PFObject inside the Store, use that instead.
		if ([aDeal.object.objectId compare:self.object.objectId] != NSOrderedSame) {
			return NO;
		}
		return YES;
	} else {
		...
		if ([aDeal.title    compare:self.title]    != NSOrderedSame
